...
my $HELP = '
        ...
        Usage:
               perl bam_split.pl    [-version]    [-help]  [-end Type]   [-in inputDir]    [-out outDir]
        For instance:
               nohup  time   perl bam_split.pl   -end paired  -in 5_finalBAM/3_STAR   -out 6_splitBAM/3_STAR    > bam_split.runLog.txt  2>&1     &      
...
## Version Infromation
my $version = "    Separate Strand Specific RNA-Seq Alignments by Strand, version 1.5,  2023-01-01.";
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl bam_split.pl  -help' \n";
    exit 0;
}
...
for(my $i=0; $i<=$#inputFiles_g; $i=$i+1) {
        ...
        print "$temp\n";
        if($SE_or_PE  eq  "single"){ system("sh   bam_split_single_end.sh     $input_g/$temp    $output_g");  }       
        if($SE_or_PE  eq  "paired"){ system("sh   bam_split_paired_end.sh     $input_g/$temp    $output_g");  } 
}
