...
targetCol = as.vector( dataFrameA[,11] )
j=1
for(i in vectorFP_1 ) {
  upstream = c((i-flanks_g-1+1):(i-2+1))
  downstream = c((i+0+1):(i+flanks_g-1+1))
  index1 = c(upstream, downstream)
  index1[index1<1] = 1
  index1[index1>length(targetCol)] = length(targetCol)
  matrixFP[j,] = as.character( targetCol[index1] )
  j = j+1
}
...
for(i in vectorTP_1 ) {
  ...
  index1[index1<1] = 1
  index1[index1>length(targetCol)] = length(targetCol)
  matrixTP[j,] = as.character( targetCol[index1] )
  j = j+1
}
