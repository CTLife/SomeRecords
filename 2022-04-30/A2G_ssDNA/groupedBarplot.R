...
p1 <- ggplot(rawMatrix_1A, aes(x=V2, y=log2(V1), fill=as.factor(V3) )) + geom_bar(stat="identity", color="black", position=position_dodge( )) +
        labs(x = "samples", y="Number of mutation sites, log2") + scale_y_continuous(limits = c(0, 22), breaks = seq(0, 22, 2) )
...
p2 <- ggplot(rawMatrix_1A, aes(x=V2, y=V1/1000000, fill=as.factor(V3) )) + geom_bar(stat="identity", color="black", position=position_dodge( )) +
        labs(x = "samples", y="Number of mutation sites (10^6)")  ## + scale_y_continuous(limits = c(0, 22), breaks = seq(0, 22, 2) )

MySaveGgplot2_1_g(ggplot2Figure1=p2, path1=outDir_g, fileName1="barPlot2", height1=4, width1=7)
