...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {
        ...
        $temp =~ s/\.bam$// or die;
        print "$temp\n";
        system( "  QTLtools mbv   --bam  $input_g/$temp.bam   --vcf otherFiles/genotype.chr1-22.vcf.gz    --out   $output_g/$temp.out    --filter-keep-duplicates    --log $output_g/$temp.Log  >   $output_g/$temp.runLog.txt  2>&1   " );                                     
}
