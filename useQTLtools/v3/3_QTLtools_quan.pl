...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {
        ...
        my $name = $temp;
        $name =~ s/\.IP//;
        system( "  QTLtools quan   --bam  $input_g/$temp.bam   --gtf  otherFiles/Peaks_27samples.sorted.gtf.gz    --sample $name   --out-prefix $output_g/$temp/$temp    --rpkm  --tpm  --xxhash   --log $output_g/$temp.Log   >   $output_g/$temp.runLog.txt  2>&1         " );                                     
}
