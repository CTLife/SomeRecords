...
my $output_g = '';  ## such as "1_bamstat/IP"
...
my $HELP = '
        ...
        ------------------------------------------------------------------------------------------------------------------------------------------------------     
        Usage:
               perl 1_QTLtools_bamstat.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]      
        For instance:
               nohup time  perl 1_QTLtools_bamstat.pl   -in 0_BAM/IP   -out 1_bamstat/IP     > 1_QTLtools_bamstat.IP.runLog.txt  2>&1    &
...
$output_g = '1_bamstat/IP';      ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl 1_QTLtools_bamstat.pl  -help' \n";
    exit 0;
}
...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {
        ...
        $temp =~ s/\.bam$// or die;
        print "$temp\n";
        system( "QTLtools bamstat   --bam $input_g/$temp.bam   --bed otherFiles/hg38.exon.chr1-22.bed.gz    --out $output_g/$temp.out   --log $output_g/$temp.Log     --filter-keep-duplicates  >   $output_g/$temp.runLog.txt  2>&1 " );                                     
}
