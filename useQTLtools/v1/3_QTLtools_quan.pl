...
my $HELP = '
        ...
        For instance:
               nohup time  perl 3_QTLtools_quan.pl   -in 0_BAM/IP   -out 3_quan/IP     > 3_QTLtools_quan.runLog.txt  2>&1    &
...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {
        ...
        system( "  QTLtools quan   --bam  $input_g/$temp.bam   --gtf  0_BAM/hg.gencode.v39.chr1-22.gtf.gz    --sample $temp   --out-prefix $output_g/$temp/$temp    --rpkm  --tpm  --xxhash   --log $output_g/$temp.Log   >   $output_g/$temp.runLog.txt  2>&1         " );                                     
}
