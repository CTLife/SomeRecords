...
my $HELP = '
        ...
        For instance:
               nohup time  perl 2_QTLtools_mbv.pl   -in 0_BAM/IP   -out 2_mbv/IP     > 2_QTLtools_mbv.runLog.txt  2>&1    &
...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {
        ...
        $temp =~ s/\.bam$// or die;
        print "$temp\n";
        system( "  QTLtools mbv   --bam  $input_g/$temp.bam   --vcf 0_BAM/out.wgs.phasing.impute.vcf.gz    --out   $output_g/$temp.out    --filter-keep-duplicates    --log $output_g/$temp.Log  >   $output_g/$temp.runLog.txt  2>&1   " );                                     
}
