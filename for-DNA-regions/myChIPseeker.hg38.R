...
MyPeaksAnno_OneGroup_1_g <- function(myPeak_t1, myPath_t1) { 
  ...
  dev.off()
  pdf(file=paste(myPath_t1, "2_ChIP_Peaks_over_Chromosomes.chromXY.pdf", sep="/"),  width=20, height=10)
...
MyAnnotation_OneGroup_1_g <- function(myFile_t1, myPath_t1) {
  ...
  #my_folder_10 <- paste(myPath_t1,  "4F-GeneOntology-1kb",  sep="/") 
  #if( ! file.exists(my_folder_10)  ) { dir.create(my_folder_10,  recursive = TRUE)  }
  #MyGeneOntology_OneGroup_1_g(MyMaxDis=1000, MyPeaksAnno=peakAnno, MyFolder=my_folder_10,  MyFileName="4F_1kb-NearestGenes")
