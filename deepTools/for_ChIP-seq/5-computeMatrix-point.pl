...
my $output_g = '';  ## such as "6-computeMatrix-point/H3K4me3"
...
my $HELP = '
        ...
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Usage:
               perl  5-computeMatrix-point.pl    [-version]    [-help]      [-in inputDir]     [-bed bedDir]     [-out outDir]
        For instance:
               perl  5-computeMatrix-point.pl    -in 3-subtractInput/H3K4me3   -bed GenomicRegions/3_Enhancers/allPups    -out 6-computeMatrix-point/H3K4me3    > 5-computeMatrix-point.H3K4me3.runLog
...
$output_g   = '6-computeMatrix-point/H3K4me3';   ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  5-computeMatrix-point.pl  -help' \n";
    exit 0;
}
...
for ( my $i=0; $i<=$#BED_Files_g; $i++ ) { 
    ...
    for ( my $j=0; $j<=$#BigWigfiles_g; $j++ ) {
         ...
         system("computeMatrix    reference-point    --scoreFileName $input_g/$BigWigfiles_g[$j]    --regionsFileName $BEDdir_g/$BED_Files_g[$i]     --smartLabels   --numberOfProcessors max/2    --verbose    -o $output_sub1/1.gzipped-matrix.gz    --outFileNameMatrix $output_sub1/1.matrix.txt   --outFileSortedRegions  $output_sub1/1.Regions.txt    --referencePoint  center   --upstream 6000   --downstream 6000  --binSize 50    --missingDataAsZero     >> $output_sub1/1.runLog.txt   2>&1");                               
