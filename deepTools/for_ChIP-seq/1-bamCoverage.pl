...
###################################################################################################################################################################################################
my $input_g  = '';  ## such as "1-BAM/H3K4me3"
my $output_g = '';  ## such as "2-bamCoverage/H3K4me3"
...
my $HELP = '
        ...
        Usage:
               perl  1-bamCoverage.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               perl  1-bamCoverage.pl    -in 1-BAM/H3K4me3   -out 2-bamCoverage/H3K4me3    > 1-bamCoverage.H3K4me3.runLog  2>&1 
...
$input_g  = '1-BAM/H3K4me3';              ## This is only an initialization value or suggesting value, not default value.
$output_g = '2-bamCoverage/H3K4me3';      ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  1-bamCoverage.pl  -help' \n";
    exit 0;
}
...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {
        ...
        $temp =~ s/.bam$//  or  die;
        system(" bamCoverage   --bam $input_g/$temp.bam     --outFileName $output_g/$temp.bw     --outFileFormat bigwig    --binSize 50    -p max/2     --verbose   --effectiveGenomeSize 2913022398   --normalizeUsing RPGC   --ignoreForNormalization chrX chrY chrM    --extendReads  --ignoreDuplicates   --minFragmentLength 100    --maxFragmentLength 1000     > $output_g/$temp.runLog.txt  2>&1  ");                                                  
}
