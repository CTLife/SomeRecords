...
## example: Rscript  group_violinplot.R  CTCF.NTR.TPM
...
###########################################################################################################################################################################
rawMatrix_1 <- read.table(inputFile, header=TRUE,   sep="",   quote = "",   comment.char = "")  

dim(rawMatrix_1 )
rawMatrix_1[1:10,]

index1 <- order(rawMatrix_1[,3])  ## sort by TPM      
length(index1)

rawMatrix_1a <- rawMatrix_1[index1,]
dim(rawMatrix_1a )
rawMatrix_1a[1:10,]

oneClass <- ceiling( length(index1)/4 )
oneClass2 <- length(index1) - 3*oneClass
fourClasses <-  c( rep("Lowest", oneClass),   rep("Low", oneClass),  rep("High", oneClass) ,  rep("Highest", oneClass2 ) ) 
length( fourClasses )

rawMatrix_1b <- cbind(rawMatrix_1a, fourClasses)
dim(rawMatrix_1b )
rawMatrix_1b[1:10,]


AllResults_g <- paste("Z-FinalFigures",   inputFile,  sep = "_")   
if( ! file.exists(AllResults_g) ) { dir.create(path=AllResults_g, recursive = TRUE) }

MyBoxViolinPlot_1(vector2=rawMatrix_1b[,4],   sampleType2=rawMatrix_1b[,9],  
                  sampleRank2=c("Lowest", "Low", "High", "Highest"),   colours2="red",   
                  path2=AllResults_g,   fileName2=paste("1-vilinplot",   "byTPM",  sep = "_"),    
                  title2="byTPM",  xLab2="TPM",  yLab2="NTR",    height2=4,   width2=3,   Ymin2=0, Ymax2=0.7) 
...
MyHypothesisTest_1(vector1=rawMatrix_1b[c((oneClass*0+1):(oneClass*1+1)),4],   vector2=rawMatrix_1b[c((oneClass*1+1):(oneClass*2+1)),4],    file1=paste(AllResults_g,  "/1A-TPM-1-vs-2.unpaired.txt",     sep = "")   )   
MyHypothesisTest_1(vector1=rawMatrix_1b[c((oneClass*1+1):(oneClass*2+1)),4],   vector2=rawMatrix_1b[c((oneClass*2+1):(oneClass*3+1)),4],    file1=paste(AllResults_g,  "/1B-TPM-2-vs-3.unpaired.txt",     sep = "")   )   
MyHypothesisTest_1(vector1=rawMatrix_1b[c((oneClass*2+1):(oneClass*3+1)),4],   vector2=rawMatrix_1b[c((oneClass*3+1):(oneClass2)),4],       file1=paste(AllResults_g,  "/1C-TPM-3-vs-4.unpaired.txt",     sep = "")   )   
...
###########################################################################################################################################################################
index1 <- order(rawMatrix_1[,4])  ## sort by NTR      
length(index1)
...
MyBoxViolinPlot_1(vector2=log2(rawMatrix_1b[,3]+1),   sampleType2=rawMatrix_1b[,9],  
                  sampleRank2=c("Lowest", "Low", "High", "Highest"),   colours2="red",   
                  path2=AllResults_g,   fileName2=paste("2-vilinplot",   "byNTR",  sep = "_"),    
                  title2="byNTR",  xLab2="NTR",  yLab2="log2(TPM+1)",    height2=4,   width2=3,   Ymin2=0, Ymax2=12) 
 


MyHypothesisTest_1(vector1=rawMatrix_1b[c((oneClass*0+1):(oneClass*1+1)),3],   vector2=rawMatrix_1b[c((oneClass*1+1):(oneClass*2+1)),3],    file1=paste(AllResults_g,  "/2A-TPM-1-vs-2.unpaired.txt",     sep = "")   )   
MyHypothesisTest_1(vector1=rawMatrix_1b[c((oneClass*1+1):(oneClass*2+1)),3],   vector2=rawMatrix_1b[c((oneClass*2+1):(oneClass*3+1)),3],    file1=paste(AllResults_g,  "/2B-TPM-2-vs-3.unpaired.txt",     sep = "")   )   
MyHypothesisTest_1(vector1=rawMatrix_1b[c((oneClass*2+1):(oneClass*3+1)),3],   vector2=rawMatrix_1b[c((oneClass*3+1):(oneClass2)),3],       file1=paste(AllResults_g,  "/2C-TPM-3-vs-4.unpaired.txt",     sep = "")   )   
