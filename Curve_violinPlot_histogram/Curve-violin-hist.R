...
###########################################################################################################################################################################
WT_500_rawMatrix1    <- read.table("H3K27ac_WT/4-1-1B-LogLinearModel_NTR.txt",           header=FALSE,   sep="",   quote = "",   comment.char = "")  
WT_500_rawMatrix2    <- read.table("H3K27ac_WT_p300/4-1-1B-LogLinearModel_NTR.txt",     header=FALSE,   sep="",   quote = "",   comment.char = "")  
WT_500_rawMatrix3    <- read.table("p300/4-1-1B-LogLinearModel_NTR.txt",              header=FALSE,   sep="",   quote = "",   comment.char = "")  

Hete_500_rawMatrix4  <- read.table("H3K27ac_WT/4-1-2A-LogLinearModel_NTR.txt",           header=FALSE,   sep="",   quote = "",   comment.char = "")  
Hete_500_rawMatrix5  <- read.table("H3K27ac_WT_p300/4-1-2A-LogLinearModel_NTR.txt",     header=FALSE,   sep="",   quote = "",   comment.char = "")  
Hete_500_rawMatrix6  <- read.table("p300/4-1-2A-LogLinearModel_NTR.txt",              header=FALSE,   sep="",   quote = "",   comment.char = "")  

CKO_500_rawMatrix7   <- read.table("H3K27ac_WT/4-1-2B-LogLinearModel_NTR.txt",           header=FALSE,   sep="",   quote = "",   comment.char = "")  
CKO_500_rawMatrix8   <- read.table("H3K27ac_WT_p300/4-1-2B-LogLinearModel_NTR.txt",     header=FALSE,   sep="",   quote = "",   comment.char = "")  
CKO_500_rawMatrix9   <- read.table("p300/4-1-2B-LogLinearModel_NTR.txt",              header=FALSE,   sep="",   quote = "",   comment.char = "")  
...
dim(CKO_500_rawMatrix8)
dim(CKO_500_rawMatrix9)
...
CKO_500_NTR8  <- CKO_500_rawMatrix8[,1]
CKO_500_NTR9  <- CKO_500_rawMatrix9[,1]
...
MyAverageLines_1(vector2= c( WT_500_NTR1, WT_500_NTR2,  WT_500_NTR3  ),   
                 numSample2=3,   
                 sampleType2=c( rep("spe1", 500),   rep("common", 500),  rep("spe2", 500)  ), 
                 sampleRank2=c(  "spe1", "common",  "spe2"  ), 
                 colours2=c(   "spe1"="purple2", "common"="blue2",  "spe2"="red2"   ), 
...
MyAverageLines_1(vector2= c( Hete_500_NTR4, Hete_500_NTR5,  Hete_500_NTR6  ),   
                 numSample2=3,   
                 sampleType2=c( rep("spe1", 500),   rep("common", 500),  rep("spe2", 500)   ), 
                 sampleRank2=c(  "spe1", "common",  "spe2"  ), 
                 colours2=c(   "spe1"="purple2", "common"="blue2",  "spe2"="red2"   ),  
...
MyAverageLines_1(vector2= c( CKO_500_NTR7, CKO_500_NTR8,  CKO_500_NTR9  ),   
                 numSample2=3,   
                 sampleType2=c( rep("spe1", 500),   rep("common", 500),  rep("spe2", 500)   ), 
                 sampleRank2=c(  "spe1", "common",  "spe2"  ), 
                 colours2=c(   "spe1"="purple2", "common"="blue2",  "spe2"="red2"   ), 
...
###########################################################################################################################################################################
WT_rawMatrix1  <- read.table("H3K27ac_WT/4-8-1B-WT-LogLinearModel_NTR.txt",   header=FALSE,   sep="",   quote = "",   comment.char = "")  
WT_rawMatrix2  <- read.table("H3K27ac_WT_p300/4-8-1B-WT-LogLinearModel_NTR.txt",     header=FALSE,   sep="",   quote = "",   comment.char = "")  
WT_rawMatrix3  <- read.table("p300/4-8-1B-WT-LogLinearModel_NTR.txt",    header=FALSE,   sep="",   quote = "",   comment.char = "")  

Hete_rawMatrix4  <- read.table("H3K27ac_WT/4-8-2A-2-EEDheto-LogLinearModel_NTR.txt",   header=FALSE,   sep="",   quote = "",   comment.char = "")  
Hete_rawMatrix5  <- read.table("H3K27ac_WT_p300/4-8-2A-2-EEDheto-LogLinearModel_NTR.txt",  header=FALSE,   sep="",   quote = "",   comment.char = "")  
Hete_rawMatrix6  <- read.table("p300/4-8-2A-2-EEDheto-LogLinearModel_NTR.txt",  header=FALSE,   sep="",   quote = "",   comment.char = "")  

CKO_rawMatrix7  <- read.table("H3K27ac_WT/4-8-2B-2-EEDko-LogLinearModel_NTR.txt",  header=FALSE,   sep="",   quote = "",   comment.char = "")  
CKO_rawMatrix8  <- read.table("H3K27ac_WT_p300/4-8-2B-2-EEDko-LogLinearModel_NTR.txt",  header=FALSE,   sep="",   quote = "",   comment.char = "")  
CKO_rawMatrix9  <- read.table("p300/4-8-2B-2-EEDko-LogLinearModel_NTR.txt",  header=FALSE,   sep="",   quote = "",   comment.char = "")  
...
MyBoxViolinPlot_1(vector2= c( WT_NTR1, WT_NTR2,  WT_NTR3  ),   
                  sampleType2=c( rep("spe1", length(WT_NTR1)),   rep("common", length(WT_NTR2)),  rep("spe2", length(WT_NTR3))   ), 
                  sampleRank2=c(  "spe1", "common",  "spe2"  ), 
                  colours2=c(   "spe1"="purple2", "common"="blue2",  "spe2"="red2"   ), 
...
MyBoxViolinPlot_1(vector2= c( Hete_NTR4, Hete_NTR5,  Hete_NTR6  ),   
                  sampleType2=c( rep("spe1", length(Hete_NTR4)),   rep("common", length(Hete_NTR5)),  rep("spe2", length(Hete_NTR6))   ), 
                  sampleRank2=c(  "spe1", "common",  "spe2"  ), 
                  colours2=c(   "spe1"="purple2", "common"="blue2",  "spe2"="red2"   ),
...
MyBoxViolinPlot_1(vector2= c( CKO_NTR7, CKO_NTR8,  CKO_NTR9  ),   
                  sampleType2=c( rep("spe1", length(CKO_NTR7)),   rep("common", length(CKO_NTR8)),  rep("spe2", length(CKO_NTR9))   ), 
                  sampleRank2=c(  "spe1", "common",  "spe2"  ), 
                  colours2=c(   "spe1"="purple2", "common"="blue2",  "spe2"="red2"   ),
...
MyHistogram_1(vector2=WT_xAxis, sampleType2=WT_Label_merge2, path2=AllResults_g,   
...
MyHistogram_1(vector2=Hete_xAxis, sampleType2=Hete_Label_merge2, path2=AllResults_g,   
...
MyHistogram_1(vector2=CKO_xAxis, sampleType2=CKO_Label_merge2, path2=AllResults_g,   
              fileName2="3C-CKO-histogram",  title2="Hete",  xLab2="NTR", height2=4,  width2=4 )
...
spe1_index <- runif(8000, min = 1, max = length(WT_NTR1) )
spe2_index <- runif(12000,  min = 1, max = length(WT_NTR3) )


###########################################################################################################################################################################
WT_NTR_merge1   <-  c( WT_NTR1[spe1_index], WT_NTR2,  WT_NTR3[spe2_index]  ) 
WT_Label_merge1 <-  c( rep("spe1", length(spe1_index)),   rep("common", length(WT_NTR2)),  rep("spe2", length(spe2_index)) )  
length(WT_NTR_merge1)
length(WT_Label_merge1)
WT_NTR_merge1[1:100]
WT_Label_merge1[1:100]
...
############################
Hete_NTR_merge1   <-  c( Hete_NTR4[spe1_index], Hete_NTR5,  Hete_NTR6[spe2_index]  ) 
Hete_Label_merge1 <-  c( rep("spe1", length(spe1_index)),   rep("common", length(Hete_NTR5)),  rep("spe2", length(spe2_index)) )  
length(Hete_NTR_merge1)
length(Hete_Label_merge1)
Hete_NTR_merge1[1:100]
Hete_Label_merge1[1:100]
...
############################
CKO_NTR_merge1   <-  c( CKO_NTR7[spe1_index], CKO_NTR8,  CKO_NTR9[spe2_index]  ) 
CKO_Label_merge1 <-  c( rep("spe1", length(spe1_index)),   rep("common", length(CKO_NTR8)),  rep("spe2", length(spe2_index)) )  
length(CKO_NTR_merge1)
length(CKO_Label_merge1)
CKO_NTR_merge1[1:100]
CKO_Label_merge1[1:100]
...
MyHistogram_1(vector2=CKO_xAxis, sampleType2=CKO_Label_merge2, path2=AllResults_g,   
              fileName2="4C-CKO-histogram",  title2="Hete",  xLab2="NTR", height2=4,  width2=4 )
