...
MyTheme_2 <- function(textSize1=14, hjust1=NULL, vjust1=NULL,  angle1=NULL) {    # "hjust=1, vjust=1, angle=30" for some boxplots.
  theme(  
    line  = element_line(colour="black",  size=1.0,   linetype=1,      lineend=NULL),                                                                                        ## all line elements.          局部优先总体,下面3个也是,只对非局部设置有效.   所有线属性.
    rect  = element_rect(colour="black",  size=1.0,   linetype=1,      fill="transparent" ),                                                                                 ## all rectangluar elements.    hjust=1: 靠右对齐.   所有矩形区域属性.
    text  = element_text(family="serif",  face="plain",  colour="black",  size=textSize1, hjust=0.5, vjust=0.5,   angle=0, lineheight=1.0,  margin = NULL, debug = NULL),    ## all text elements.           "serif" for a serif font. 所有文本相关属性.
    ...
    legend.background    = element_rect(colour="transparent", size=0, linetype=1, fill="transparent" ), 	      ## background of legend (element_rect; inherits from rect)
    legend.spacing       = grid::unit(0, "mm", data=NULL), 	                                                    ## extra space added around legend (unit). linetype=1指的是矩形边框的类型.
    legend.key           = element_rect(colour="transparent", size=0, linetype=1, fill="transparent" ), 	      ## background underneath legend keys. 图例符号. size=1指的是矩形边框的大小.
    legend.key.size      = grid::unit(0,   "mm", data=NULL) , 	                                                ## size of legend keys   (unit; inherits from legend.key.size)
    legend.key.height    = grid::unit(0, "mm", data=NULL) , 	                                                ## key background height (unit; inherits from legend.key.size)
    legend.key.width     = grid::unit(0,   "mm", data=NULL) ,                                                   ## key background width  (unit; inherits from legend.key.size)
    legend.text          = element_text(family="serif", face=NULL, colour="black", size=textSize1, hjust=NULL, vjust=NULL, angle=NULL, lineheight=NULL), 	##legend item labels. 图例文字标签.
    legend.text.align    = 0, 	                    ## alignment of legend labels (number from 0 (left) to 1 (right))
    legend.title         = element_blank(),   	    ## title of legend (element_text; inherits from title)
    legend.title.align   = 0, 	                    ## alignment of legend title (number from 0 (left) to 1 (right))
    legend.position      = "none", 	              ## the position of legends. ("left", "right", "bottom", "top", or two-element numeric vector)
    legend.direction     = "none",        	    ## layout of items in legends  ("horizontal" or "vertical")   图例排列方向
    legend.justification = "none",      	        ## anchor point for positioning legend inside plot ("center" or two-element numeric vector)  图例居中方式
    legend.box           = NULL, 	                  ## arrangement of multiple legends ("horizontal" or "vertical")  多图例的排列方式
    legend.box.just      = NULL, 	                  ## justification of each legend within the overall bounding box, when there are multiple legends ("top", "bottom", "left", or "right")  多图例的居中方式
...
###########################################################################################################################################################################
rawMatrix_1 <- read.table("Merge.sortNTR", header=TRUE,   sep="",   quote = "",   comment.char = "")  

dim(rawMatrix_1 )
rawMatrix_1[1:10,]

rawMatrix_2 <- ddply(rawMatrix_1,  .(class, class2),  summarize, value = mean(TPM))
rawMatrix_2
dim(rawMatrix_2 )
...
write.table(x=rawMatrix_2,  file=paste(AllResults_g, "/rawMatrix_2.txt", sep="") , quote = FALSE, sep="\t")     


rawMatrix_3 <- read.table(paste(AllResults_g, "/rawMatrix_2A.txt", sep=""), header=TRUE,   sep="",   quote = "",   comment.char = "")  
rawMatrix_3
dim(rawMatrix_3 )

FigureTemp2 <- ggplot(data = rawMatrix_3 , aes(x = class2,  y = log2(value), colour=class, group=class)  ) + geom_point(size=3) + geom_line() + 
  scale_x_discrete(limits= c("Lowest", "Low", "High", "Highest") ) + 
  scale_colour_manual( values=c("GATA4_spe"="purple2", "Nkx25_spe"="red2", "GATA4_Nkx25_common"="blue2" ) ) +
  MyTheme_1( hjust1=NULL, vjust1=NULL,  angle1=NULL,   textSize=14 ) 

MySaveGgplot2_1(ggplot2Figure1=FigureTemp2,  path1=AllResults_g, fileName1=paste("1-Boxplot",   "byNTR",  sep = "_"),  height1=3.5,  width1=5.5)
...
FigureTemp2 <- ggplot(data = rawMatrix_3 , aes(x = class2,  y = log2(value), colour=class, group=class)  ) + geom_point(size=3) + geom_line() + 
  scale_x_discrete(limits= c("Lowest", "Low", "High", "Highest") ) + 
  scale_colour_manual( values=c("GATA4_spe"="purple2", "Nkx25_spe"="red2", "GATA4_Nkx25_common"="blue2" ) ) +
  MyTheme_2( hjust1=NULL, vjust1=NULL,  angle1=NULL,   textSize=14 ) 

MySaveGgplot2_1(ggplot2Figure1=FigureTemp2,  path1=AllResults_g, fileName1=paste("1A-Boxplot",   "byNTR",  sep = "_"),  height1=3,  width1=3)
...
###########################################################################################################################################################################
rawMatrix_1 <- read.table("Merge.sortTPM", header=TRUE,   sep="",   quote = "",   comment.char = "")  

dim(rawMatrix_1 )
rawMatrix_1[1:10,]

rawMatrix_2 <- ddply(rawMatrix_1,  .(class, class2),  summarize, value = mean(NTR))
rawMatrix_2
dim(rawMatrix_2 )
...
write.table(x=rawMatrix_2,  file=paste(AllResults_g, "/rawMatrix_3.txt", sep="") , quote = FALSE, sep="\t")     


rawMatrix_3 <- read.table(paste(AllResults_g, "/rawMatrix_3.txt", sep=""), header=TRUE,   sep="",   quote = "",   comment.char = "")  
rawMatrix_3
dim(rawMatrix_3 )

FigureTemp2 <- ggplot(data = rawMatrix_3 , aes(x = class2,  y = value, colour=class, group=class)  ) + geom_point(size=3) + geom_line() + 
  scale_x_discrete(limits= c("Lowest", "Low", "High", "Highest") ) + 
  scale_colour_manual( values=c("GATA4_spe"="purple2", "Nkx25_spe"="red2", "GATA4_Nkx25_common"="blue2") ) +
  MyTheme_1( hjust1=NULL, vjust1=NULL,  angle1=NULL,   textSize=14 ) 

MySaveGgplot2_1(ggplot2Figure1=FigureTemp2,  path1=AllResults_g, fileName1=paste("2-Boxplot",   "byTPM",  sep = "_"),  height1=3.5,  width1=5.5)
...
MySaveGgplot2_1(ggplot2Figure1=FigureTemp2,  path1=AllResults_g, fileName1=paste("2A-Boxplot",   "byTPM",  sep = "_"),  height1=3,  width1=3)
