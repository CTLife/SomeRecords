...
#   input_g =  "Number_DMRs_diffPeaks_DEGs.txt"
#print(input_g) 
...
MyHistogram_1 <- function(x1, y1, yticks,  mytype1,    path2,   fileName2,  title2,  xLab2,  height2=4,  width2=4 ) {
  if( ! file.exists(path2)   ) { dir.create(path2, recursive = TRUE) }
  dataframeB  <- data.frame(list( xAxis = x1,  yAxis =y1,  Type1=mytype1  )  ) 
  
  FigureTemp1 <- ggplot( data=dataframeB,  aes(x=xAxis, y=yAxis, fill=Type1 ) )  +   
    ...
    MyTheme_1_g( hjust1=1, vjust1=1,  angle1=60,   textSize=30 )   
...
MyHistogram_2 <- function(x1, y1,  mytype1,    path2,   fileName2,  title2,  xLab2,  height2=4,  width2=4 ) {
  if( ! file.exists(path2)   ) { dir.create(path2, recursive = TRUE) }
  dataframeB  <- data.frame(list( xAxis = x1,  yAxis =y1,  Type1=mytype1   )  ) 
...
MyHistogram_3 <- function(x1, y1,  mytype1,   path2,   fileName2,  title2,  xLab2,  height2=4,  width2=4 ) {
  if( ! file.exists(path2)   ) { dir.create(path2, recursive = TRUE) }
  dataframeB  <- data.frame(list( xAxis = x1,  yAxis =y1,  Type1=mytype1   )  ) 
  
  FigureTemp1 <- ggplot( data=dataframeB,  aes(x=xAxis, y=yAxis, fill=Type1 ) )  +   
    geom_bar(stat="identity",  position="stack", width=0.8  ) +  
    xlab(xLab2) + ylab("Frequency") +  ggtitle(title2)  +  
    MyTheme_1_g( hjust1=1, vjust1=1,  angle1=60,   textSize=30 )   
...
###################
rawMatrix_1 <- read.table("RRBS_neonates.txt", header=FALSE, sep="\t", quote = "", comment.char = "") 
rawMatrix_2 <- read.table("4.txt", header=FALSE, sep="\t", quote = "", comment.char = "") 
rawMatrix_3 <- read.table("5.txt", header=FALSE, sep="\t", quote = "", comment.char = "") 
rawMatrix_4 <- read.table("6.txt", header=FALSE, sep="\t", quote = "", comment.char = "") 
rawMatrix_5 <- read.table("7.txt", header=FALSE, sep="\t", quote = "", comment.char = "") 
...
rawMatrix_merge = cbind(rawMatrix_1[,2], rawMatrix_2[,2], rawMatrix_3[,2], rawMatrix_4[,2], rawMatrix_5[,2], 
                        rowMeans( cbind(rawMatrix_3[,2], rawMatrix_4[,2], rawMatrix_5[,2]) ) )

vector_ratio = as.numeric( unlist(rawMatrix_merge) )
vector_type1 =  rep( c("1RRBS", "2a", "3a", "4a", "5a", "6a"), each=5 )
vector_type2 =  rep( c("1Promoter", "2Exon", "3Intron", "4Intergenic", "5Others"), times=6 )
length(vector_ratio)
length(vector_type1)
length(vector_type2)

MyHistogram_3(x1 = vector_type1, y1 = vector_ratio,  
              mytype1 = vector_type2,  
              path2 = outDir_g,   fileName2 = "Histogram-1",  title2 = "",  
              xLab2 = "Comparisons",  height2=6,  width2=7 )


rawMatrix_merge2 = cbind(rawMatrix_1[,2]*400000, rawMatrix_2[,2]*2000, rawMatrix_3[,2]*300, rawMatrix_4[,2]*300, rawMatrix_5[,2]*200, 
                        rowMeans( cbind(rawMatrix_3[,2], rawMatrix_4[,2], rawMatrix_5[,2]) )*300 )
rawMatrix_merge2 = round(rawMatrix_merge2/100)
 

sink(  paste(outDir_g, "1-vs-2.txt", sep="/") )
   aaa = stats::chisq.test( rawMatrix_merge2[,c(1,2)] , simulate.p.value = FALSE )   
   aaa$p.value
sink() 

sink(  paste(outDir_g, "1-vs-3.txt", sep="/") )
aaa = stats::chisq.test( rawMatrix_merge2[,c(1,3)] , simulate.p.value = FALSE )   
aaa$p.value
sink() 

sink(  paste(outDir_g, "1-vs-4.txt", sep="/") )
aaa = stats::chisq.test( rawMatrix_merge2[,c(1,4)] , simulate.p.value = FALSE )   
aaa$p.value
sink() 

sink(  paste(outDir_g, "1-vs-5.txt", sep="/") )
aaa = stats::chisq.test( rawMatrix_merge2[,c(1,5)] , simulate.p.value = FALSE )   
aaa$p.value
sink() 

sink(  paste(outDir_g, "1-vs-6.txt", sep="/") )
aaa = stats::chisq.test( rawMatrix_merge2[,c(1,6)] , simulate.p.value = FALSE )   
aaa$p.value
sink() 


#############
sink(  paste(outDir_g, "2-vs-3.txt", sep="/") )
aaa = stats::chisq.test( rawMatrix_merge2[,c(2,3)] , simulate.p.value = FALSE )   
aaa$p.value
sink() 

sink(  paste(outDir_g, "2-vs-4.txt", sep="/") )
aaa = stats::chisq.test( rawMatrix_merge2[,c(2,4)] , simulate.p.value = FALSE )   
aaa$p.value
sink() 

sink(  paste(outDir_g, "2-vs-5.txt", sep="/") )
aaa = stats::chisq.test( rawMatrix_merge2[,c(2,5)] , simulate.p.value = FALSE )   
aaa$p.value
sink() 

sink(  paste(outDir_g, "2-vs-6.txt", sep="/") )
aaa = stats::chisq.test( rawMatrix_merge2[,c(2,6)] , simulate.p.value = FALSE )   
aaa$p.value
sink() 
