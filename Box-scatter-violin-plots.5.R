...
###########################################################################################################################################################################
DF_1 <- read.table("All.txt-merged.bed-BA9.genotype.txt", header=F,   sep="\t" )  
dim(DF_1 )
 #  DF_1[1:10,] 
...
myBool = ( (DF_gno_times[,6] >= 3) & (abs(distance) < 500000) )
length(myBool )
...
write.table(x=DF_1, file = paste(AllResults_g, "Selected.all.txt", sep="/"), append = FALSE, quote = FALSE, sep = "\t",
...
My_Box_Violin_Scatte_Plot(vector2=as.vector(DF_phe[i,]),   sampleType2=as.vector(DF_gno[i,]),    
                          ...
                          title2=myname,  xLab2="Genotype",  yLab2="Phenotype (normalzied m6A level)",   
...
My_Box_Violin_Scatte_Plot(vector2=as.vector(DF_phe[i,]),   sampleType2=as.vector(DF_gno[i,]),    
                          ...
                          title2=myname,  xLab2="Genotype",  yLab2="Phenotype (normalzied m6A level)",   
                          height2=5,  width2=4.5,   Ymin2=-4, Ymax2=4, alpha2=1)
