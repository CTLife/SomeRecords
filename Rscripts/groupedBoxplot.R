...
###########################################################################################################################################################################
Housekeeping_1    <- read.table("Housekeeping_NTR_TPM",  header=TRUE,   sep="",   quote = "",   comment.char = "")  
Specific_2        <- read.table("Specific_NTR_TPM",      header=TRUE,   sep="",   quote = "",   comment.char = "")  
...
MyBoxViolinPlot_1(vector2=  c( Housekeeping_1[,6],   Specific_2[,6]  ) ,   
                  ...
                  sampleRank2=c(  "Housekeeping",   "HeartSpecific"  ), 
                  colours2=c(   "Housekeeping"="red2",    "HeartSpecific"="blue2"   ), 
                  path2=AllResults_g,   fileName2= paste("2-BoxViolin-all",   "NTR",  sep = "_") ,  
...
WT_oneClass <- ceiling( length(Housekeeping_1[,3])/4 )
WT_oneClass
WT_oneClass2 <- length(Housekeeping_1[,3]) - WT_oneClass * 3
WT_xAxis <-  c( rep("Lowest", WT_oneClass),   rep("Low", WT_oneClass),  rep("High", WT_oneClass) ,  rep("Highest", ceiling(WT_oneClass ) )  ) 
length(WT_xAxis)
...
class4 <- c(WT_oneClass*3+1: WT_oneClass*4)
...
MyBoxViolinPlot_1(vector2=  c( Housekeeping_1[class1,6],   Housekeeping_1[class2,6],    Housekeeping_1[class3,6],   Housekeeping_1[class4,6]   ) ,   
                  ...
                  colours2= "red2", 
                  path2=AllResults_g,   fileName2= paste("3-BoxViolin-4class",   "NTR",  sep = "_") ,  
                  title2="Genes",   xLab2="TPM",    yLab2="NTR",   
                  height2=4.0,   width2=3,   Ymin2=0, Ymax2=1.0)    ## width = 1 + 4*0.5, height=5cm 
...
B_WT_oneClass
B_WT_oneClass2 <- length(Specific_1[,3]) - B_WT_oneClass * 3
B_WT_xAxis <-  c( rep("Lowest", B_WT_oneClass),   rep("Low", B_WT_oneClass),  rep("High", B_WT_oneClass) ,  rep("Highest", ceiling(B_WT_oneClass ) )  ) 
length(B_WT_xAxis)
length(Specific_1[,3])
...
B_class4 <- c(B_WT_oneClass*3+1: B_WT_oneClass*4)
...
MyBoxViolinPlot_1(vector2=  c( Specific_2[B_class1,6],   Specific_2[B_class2,6],    Specific_2[B_class3,6],   Specific_2[B_class4,6]   ) ,   
                  ...
                  sampleRank2=c(  "Lowest",   "Low", "High",  "Highest"), 
                  colours2= "blue2", 
                  path2=AllResults_g,   fileName2= paste("4-BoxViolin-SpecificGenes",   "NTR",  sep = "_") ,  
                  title2="Genes",   xLab2="TPM",    yLab2="NTR",   
                  height2=4.0,   width2=3,   Ymin2=0,    Ymax2=1.0)    ## width = 1 + 4*0.5, height=5cm 
...
MyHypothesisTest_1(vector1=WT_NTR1,   vector2=WT_NTR2,    file1=paste(AllResults_g,  "/1A-WT-spe1-vs-common.unpaired.txt",     sep = "")   )   
MyHypothesisTest_1(vector1=WT_NTR1,   vector2=WT_NTR3,    file1=paste(AllResults_g,  "/1B-WT-spe1-vs-spe2.unpaired.txt",  sep = "")   )   
MyHypothesisTest_1(vector1=WT_NTR2,   vector2=WT_NTR3,    file1=paste(AllResults_g,  "/1C-WT-common-vs-spe2.unpaired.txt",  sep = "")   )   

MyHypothesisTest_1(vector1=Hete_NTR4,   vector2=Hete_NTR5,    file1=paste(AllResults_g,  "/2A-Hete-spe1-vs-common.unpaired.txt",     sep = "")   )   
MyHypothesisTest_1(vector1=Hete_NTR4,   vector2=Hete_NTR6,    file1=paste(AllResults_g,  "/2B-Hete-spe1-vs-spe2.unpaired.txt",  sep = "")   )   
MyHypothesisTest_1(vector1=Hete_NTR5,   vector2=Hete_NTR6,    file1=paste(AllResults_g,  "/2C-Hete-common-vs-spe2.unpaired.txt",  sep = "")   )   

MyHypothesisTest_1(vector1=CKO_NTR7,   vector2=CKO_NTR8,    file1=paste(AllResults_g,  "/3A-CKO-spe1-vs-common.unpaired.txt",     sep = "")   )   
MyHypothesisTest_1(vector1=CKO_NTR7,   vector2=CKO_NTR9,    file1=paste(AllResults_g,  "/3B-CKO-spe1-vs-spe2.unpaired.txt",  sep = "")   )   
MyHypothesisTest_1(vector1=CKO_NTR8,   vector2=CKO_NTR9,    file1=paste(AllResults_g,  "/3C-CKO-common-vs-spe2.unpaired.txt",  sep = "")   )   
...
merge_1    <- read.table("merge",  header=TRUE,   sep="",   quote = "",   comment.char = "")  
dim(merge_1)
merge_1[1:10,]

Length_oneClass <- ceiling( length(merge_1[,3])/4 )
length(Length_oneClass)
my_xAxis <-  c( rep("ALowest", Length_oneClass),   rep("BLow", Length_oneClass),  rep("CHigh", Length_oneClass) ,  rep("DHighest", Length_oneClass-2 )  ) 
length(my_xAxis)
length(merge_1[,3])
...
merge_2  <- cbind(merge_1, my_xAxis)
dim(merge_2)
merge_2[1:10,]


FigureTemp1 <- ggplot( merge_2, aes(x=my_xAxis, colour=type) ) + scale_x_discrete(limits= c(  "Lowest",   "Low", "High",  "Highest") )  +
  geom_errorbar( aes(ymin=0, ymax=1),  data=whisk_1(merge_2),   width=0.2, size=0.5 ) +
  geom_boxplot( width=0.6,   aes(y=NTR.max), outlier.colour="gray45",  outlier.shape=1, outlier.size=0.1, size=0.5 ) +    
  xlab("TPM" ) + ylab( "NTR" ) + ggtitle( "genes" )  + MyTheme_1(textSize1=14, hjust1=1, vjust1=1,  angle1=30 ) + ylim(0, 1 )
MySaveGgplot2_1(ggplot2Figure1=FigureTemp1,  path1=AllResults_g, fileName1= paste("5-BoxViolin",   "NTR",  sep = "_") ,  height1=4, width1=3)
...
merge_2[,6]  <- abs(merge_2[,6]) 
merge_2[,3]  <- log2( abs(merge_2[,3]) + 1)
merge_2[,3][merge_2[,3]>12] <- 12
merge_2[,6][merge_2[,6]>1] <- 1

pdf("1.pdf")
p1 <- plot_ly(merge_2, x = ~my_xAxis, y = ~NTRmax, color = ~type2, colors = c("Housekeeping"="red2",    "HeartSpecific"="blue2" ),   type = "box") %>% layout(boxmode = "group")
print(p1)
dev.off()
...
plot_ly(merge_2, x = ~my_xAxis, y = ~TPM, color = ~type2, colors = c("Housekeeping"="red2",    "HeartSpecific"="blue2" ),   type = "box") %>% layout(boxmode = "group")
...
FigureTemp2 <- ggplot(data = merge_2, aes(y = NTRmax, x = my_xAxis, fill = type2)  ) + geom_boxplot() +
               MyTheme_1( hjust1=NULL, vjust1=NULL,  angle1=NULL,   textSize=14 ) 

MySaveGgplot2_1(ggplot2Figure1=FigureTemp2,  path1=AllResults_g, fileName1=paste("5-BoxViolin",   "NTR",  sep = "_"),  height1=4,  width1=5)




FigureTemp2 <- ggplot(data = merge_2, aes(y = TPM, x = my_xAxis, fill = type2)  ) + geom_boxplot() +
  MyTheme_1( hjust1=NULL, vjust1=NULL,  angle1=NULL,   textSize=14 ) 

MySaveGgplot2_1(ggplot2Figure1=FigureTemp2,  path1=AllResults_g, fileName1=paste("5-BoxViolin",   "TPM",  sep = "_"),  height1=4,  width1=5)
