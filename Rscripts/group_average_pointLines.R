...
###########################################################################################################################################################################
rawMatrix_1 <- read.table("Merge.TPM.NTR", header=TRUE,   sep="",   quote = "",   comment.char = "")  

dim(rawMatrix_1 )
rawMatrix_1[1:10,]

rawMatrix_2 <- ddply(rawMatrix_1,  .(NTR2, class),  summarize, value = mean(TPM))
rawMatrix_2
dim(rawMatrix_2 )
...
FigureTemp2 <- ggplot(data = rawMatrix_3 , aes(x = NTR2,  y = log2(value), colour=class, group=class)  ) + geom_point() + geom_line() + 
  scale_x_discrete(limits= c("Lowest", "Low", "High", "Highest") ) + 
  scale_colour_manual( values=c("GATA4_spe"="red2", "Nkx25_spe"="orange2", "Tbx20_spe"="yellow3", "GATA4_Nkx25_spe"="green2", 
                                "GATA4_Tbx20_spe"="blue2", "Nkx25_Tbx20_spe"="cyan2", "GATA4_Nkx25_Tbx20_common"="purple2"),  
                       breaks=c("GATA4_spe", "Nkx25_spe", "Tbx20_spe", "GATA4_Nkx25_spe", "GATA4_Tbx20_spe", "Nkx25_Tbx20_spe", "GATA4_Nkx25_Tbx20_common") ) +
  MyTheme_1( hjust1=NULL, vjust1=NULL,  angle1=NULL,   textSize=14 ) 

MySaveGgplot2_1(ggplot2Figure1=FigureTemp2,  path1=AllResults_g, fileName1=paste("1-Boxplot",   "byNTR",  sep = "_"),  height1=3.5,  width1=7)
